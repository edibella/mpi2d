...
% MID=107;
% if doReconFlag==1
%     load(['/v/raid1/jfluck/MRIdata/Cardiac/Verio/' Patient '/RawData/meas_MID107_CV_Radial7Off_flex_72rays1_5_FID76952_Kspace.mat'],'-mat');
% 
% end
% % size(kSpace)
% %    256   72 15 160 2   for P040110  and series 18
% % 
% for seriesnum=(seriesnumStart):seriesnumStart%seriesnumEnd
...
%     output_dicom_directory=strcat('/v/raid1/bmatthew/MRIdata/Cardiac/',scanner,'/',Patient,'/tempReconData/',scan_name );
%     ed_s_subsetRecon_directory=strcat('/v/raid1/ed/MRIdata/Cardiac/',scanner,'/',Patient,'/ReconData/',scan_name );
...
%         for subset=1:3
%             % get directory name
%             dirname=strcat(ed_s_subsetRecon_directory,int2str(seriesnum*1000+subset),')/')
%             files=dir(strcat(dirname,'*.dcm'))
%             for bb=1:length(files) 
%                 tmpp=dicomread( strcat(dirname,files(bb).name));
%                 %tmpp=flipud(tmpp); 
%                 %imgrr(:,:,bb)=rot90(tmpp);
%                 imgrr(:,:,bb)=tmpp;%rot90(tmpp);
%             end
% 
%             seriesdesc = outname;
...
seriesnumEnd=25
% must also change filenames
scan_name='CV_Radial7Off_flex_72rays2.5('
MID=110
if doReconFlag==1
    load(['/v/raid1/jfluck/MRIdata/Cardiac/Verio/' Patient '/RawData/meas_MID110_CV_Radial7Off_flex_72rays2.5_FID76955_Kspace'],'-mat');
...
for seriesnum=seriesnumStart:seriesnumEnd
    ...
    if flipFlag
        for subset=1:3
            ...
            seriesdesc_out = strcat(seriesdesc,int2str(subset),'_window',int2str(10*modifyWindowSize));
            % write out as dicom:
            mkdir([output_dicom_directory num2str(seriesnum*1000+subset) ')']);
            func_dicomHeader_OrderByTime_SkipFirst(template_dicom_directory, imgrr, [output_dicom_directory num2str(seriesnum*1000+subset) ')/' outname num2str(subset) '_window' int2str(10*modifyWindowSize) '_'], 1000*seriesnum+subset, seriesdesc_out);
        end
    end
    if doProcessing
...
seriesnumEnd=32
% must also change filenames
scan_name='CV_Radial7Off_flex_72rays3.7adeno('
MID=113
if doReconFlag==1
    load(['/v/raid1/jfluck/MRIdata/Cardiac/Verio/' Patient '/RawData/meas_MID113_CV_Radial7Off_flex_72rays3.7adeno_FID76958_Kspace'],'-mat');
end
...
seriesnumEnd=53
% must also change filenames
scan_name='CV_Radial7Off_flex_72rays3.7lexi('
MID=141
if doReconFlag==1
    load(['/v/raid1/jfluck/MRIdata/Cardiac/Verio/' Patient '/RawData/meas_MID141_CV_Radial7Off_flex_72rays3.7lexi_FID76986_Kspace'],'-mat');
