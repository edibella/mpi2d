function mpi_remakeOriginalWithFits(img,sliceNum, studyNum, outpath,numAzimuthalRegions, numRadialRegions, referenceFrame, flagPixelwise, numPreContrast_bld, numPreContrast_tiss)

pixelSizeRatio=1;   % should make so is read in...
...
%kwi=est_fb;   % TEMPORARYU TO WRITE OUT FV!!
...
%   for j = 1 : size(img,1)
%    for k = 1 : size(img,2)
%      counter=(j-1)*size(img,2)+k;
%      [fittedcurve, tmpbldcurve]=mpi_fwdModel(bldcurve, delta_t, kwi(counter), kwo(counter), t_delay(counter), est_fb(counter));
%      fit_img(j,k,:) = 10*fittedcurve;
%    end
%   end
...
%   fit_img(:,:,ii) = fit_img(:,:,ii).*scale_img;
%   fit_img(:,:,ii) = init_img + fit_img(:,:,ii);
end
...
%   outfilename=strcat(outpath,'img.study',int2str(studyNum),'.slice',int2str(sliceNum),'newOriginal.pixelwise.fv.',int2str(ncols),'x',int2str(nrows),'.flt');
...
ff2=fopen(outfilename2,'w','ieee-be')
for i=1:nFrames
   fwrite(ff2,100*img(:,:,i)','short');
%  fwrite(ff2,fit_img(:,:,i)','float');
end
