function deltaSIcurves=mpi_useAlternateAIF(seriesNumAIF, sliceNumAIF, scaleAIF, sliceNum,seriesNum, outpath, numPreContrast_bld, numPreContrast_tiss, numSkip)
...
% read in alternate AIF 
   ...
   try 
       ...
       disp('cant seem to find curves.series file to load, may not have run 3.2 yet')
   end
...
%   bldcurve=aif_curve*10*init_bld/init_aif;
   ...
   % Here I'll scale the alternate blood curves using the SI values from
   % the tails of the AIF and original blood curves
   t11=length(aif_curve)-6; t12=length(aif_curve)-1;
   t21=length(bldcurve)-6; t22=length(bldcurve)-1;
   
   init_aif=mean(aif_curve(t11:t12));% also see mpi_useAlternateAIFtail_FROM_TAEHO.m  
   init_bld=mean(bldcurve(t21:t22));% also see mpi_useAlternateAIFtail_FROM_TAEHO.m
...
scale_to_AIF=1; % set to 0 when scaling AIF directly (when volume matched) NP 041709 % this is set to 1 only if scaling AIF from another slice
add_inputsFlag=0;
summedAIF=0;

if (scale_to_AIF==1) 
% don't scale up by 10, just scale resulting param. ktrans by 10...
   %bldcurve(1:nTimesAIF)=aif_curve*init_bld/init_aif; %% Now using scale factor from tails here NP 082609
   bldcurve(1:nTimesAIF)=aif_curve*abs(tailscale); % so not negative
   disp('scaling aif by init_bld/init_aif NOT times 10')
   %init_bld/init_aif
   if(abs(init_bld/init_aif)< 0) % so not negative (see above too)
       disp('Ratio between blood and AIF is negative');
   end
else
