function processUngated(raid1path, seriesName, first_series, numSlices, saveflag);
...
call_autoROI(raid1path, seriesName_tmp, islice, iseries, saveflag);

cd([raid1path,'/Processing/']);

call_sys_dias_modifyParFile(raid1path, islice, iseries);
...
function call_sys_dias_modifyParFile(raid1path, islice, iseries)
...
for isub=90:93  % low dose sys, low dose dias, high sys, high dias
   ...
   fidin = fopen(templateParFile);
    ...
    while 1
        ...
        if(~isempty(strfind(tline,'framesToSkip')))
...
function call_autoROI(raid1path, seriesName, islice, iseries, saveflag)
...
fullcinemri1=sos(:,:,100:round(bb/2));  % skip pre-bolus part, assuming it is less reliable?  Actually might be better! Should try. 
fullcinemri1=sos;
