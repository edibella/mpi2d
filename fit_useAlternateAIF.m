function fitparams = fit_useAlternateAIF(seriesNumAIF, sliceNumAIF, scaleAIF, sliceNum,studyNum,outpath,filenamePart1,delta_t, numAzimuthalRegions, numRadialRegions, flagPixelwise, numSkip, curvesFromClusters, fixedVp, fixedDelay, timeStampFile, timeStampFileAIF)
...
% add 10/6/05 to cut off frames? 
%ranget=1:30;
%bldcurve=bldcurve(ranget);
%sat_bldcurve=sat_bldcurve(ranget);
%tisscurve=tisscurve(ranget,:);
 %nTimes=length(bldcurve);
...
%X0(5)=0.0;  LB(5)=-15.0; UB(5)=0.8;    % t_delayVp 
...
%for this one off, sample bldcurve to first time stamp file, 
% and  sample tiss. to second
...
estGlobalDelay=x(3)
   ...
   X0(1)=1.0; LB(1)=0.01*X0(1); UB(1)=10*X0(1); 
   X0(2)=0.5; LB(2)=0.01*X0(2); UB(2)=20*X0(2); 
