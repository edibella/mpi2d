...
%pause
   bldcurve = interp1(origSampleTimes, bldcurve, delta_t*(0:nTimes-1),'cubic',0)  % changed from splines, EVRD 10/2010
   bldcurve=bldcurve';
   for ii=1:nRegs
      tisscurve(:,ii) = interp1(origSampleTimes,tisscurve(:,ii),delta_t*(0:nTimes-1),'cubic',0);
   end
...
N=5 % since delta_t=0.5 seconds, seems need more. EVRD 10/2010.  Was 3;    % Schwitter2001 used 3 point linear fit in blood and 5 point in tissue
...
N=9;  % since delta_t=0.5 seconds, seems need more. EVRD 10/2010.  Was 5;
xvec=1:N; xvec=xvec';
maxslope=zeros(1,nRegs);
slope_location=zeros(1,nRegs);
for ireg=1:nRegs
% linear fit of N points:   % see Al Saadi Circ 2000 and Scwhitter Circ 2001
   for i=numSkipUpslope:nTimes-2*N  % EVRD 10/2010,   was nTimes/2-N      % make sure nTimes even
...
figure(2);clf; hold on
   ...
   disp('region is '),ireg
   disp('Time frame each one is 0.5 seconds if this is time-stamp corrected.. ')
...
% %maxslope=maxslope./maxslope_bld;  % changed below to NOT when commented
% %this EVRD  10/23/10
...
% 
% fprintf(fid,'\nAbove NOT already normalized by blood slope which is %f  at index %d\n',maxslope_bld, slope_location_bld);
% fclose(fid);

 upslopes.m = maxslope;  % / AIFmMax;
    upslopes.t = slope_location;  %delays;
    save(['Output/upslopes.study' int2str(studyNum) '.slice' int2str(sliceNum) '.mat'],'upslopes');
