function ReconMat(MID, seriesNum,slice,OrientationTokens,coils)
    ...
    clear img_est
    if(~isempty(strfind(rawMatFiles(1).name,'Coil')))
    ...
    func_dicomHeader(dicomFolder, imgrr, [ReconDatawd '/'], seriesNum*1000, strrep(targetDicomDataFolder,['(' num2str(seriesNum) ')'],''));
    cd(Processingwd);
end