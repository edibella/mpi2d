...
scanner='Trio'
Patient='P071709'
types ={'high','adeno','lexi'};  % did ahve 'low' first
MIDs = [83 85 109];
OrientationTokens = {'nothing'};
...
processingDir = ['/v/raid1/ed/MRIdata/Cardiac/' scanner '/' Patient '/Processing'];
...
for MIDi = 1:length(MIDs)
    ...
    for seriesnum=seriesnumStart:seriesnumEnd
        ...
        if(~isempty(OrientationTokens))
            for subset=1:3
                % get directory name
                dirname=[output_dicom_directory int2str(seriesnum*1000+subset) ')/'];
                files=dir([dirname '*.dcm']);
                tmpp=dicomread( strcat(dirname,files(1).name));
                imgrr = zeros([size(tempp),length(files)]);
                for t=1:length(files) 
                    tmpp=dicomread( [dirname files(t).name]);
                    for opperationi = 1:length(OrientationTokens)
                        operation = OrientationTokens{opperationi};
                        if(strcmp(operation,'flipud'))
                            imgrr(:,:,t) = flipud(tmpp); %#ok<*AGROW>
                        end
                        ...
                        if(strcmp(operation,'rot-90'))
                            imgrr(:,:,t) = rot90(tmpp,-1);
                        end
                    end
                end

                seriesdesc = outname;
                seriesdesc_out = [seriesdesc int2str(subset) '_window' int2str(10*modifyWindowSize)];
                % write out as dicom:
                par1 = [output_dicom_directory num2str(seriesnum*1000+subset) ')/' outname num2str(subset) '_window' int2str(10*modifyWindowSize) '_'];
                func_dicomHeader_OrderByTime_SkipFirst(template_dicom_directory, imgrr,par1 , 1000*seriesnum+subset, seriesdesc_out);
            end
        end
        if doProcessing
