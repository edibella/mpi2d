...
scanner='Trio'
Patient='P090309'
MRIdataOwner = 'bmatthew';   %Owner of the ReconData folder
startFrame=1
...
flipFlag = 1;
doReconFlag=1;  % 1: do reconstruction, 0: don't do reconstruction
...
scan_name='CV_Radial7Off_triple_1_5(';
MID=51;

if doReconFlag==1
    load(['/v/raid1/bmatthew/MRIdata/Cardiac/' scanner '/' Patient '/RawData/meas_MID51_CV_Radial7Off_triple_1_5_FID11089_Kspace.mat'],'-mat');
end
...
for seriesnum=seriesnumStart:seriesnumEnd
    ...
    template_dicom_directory=strcat('/v/raid1/', MRIdataOwner, '/MRIdata/Cardiac/',scanner,'/',Patient,'/DicomData/',scan_name,int2str(seriesnum),')')
    output_dicom_directory=strcat('/v/raid1/', MRIdataOwner, '/MRIdata/Cardiac/',scanner,'/',Patient,'/ReconData/Subsets/',scan_name );
    ...
    if(review)
        review = 0;
        disp('Please load a sample dicom file and confirm that orientation is kosher');
        keyboard;
    end
    if doProcessing
        % get current directory so we can return to it afterwards
        cd(strcat('/v/raid1/', MRIdataOwner, '/MRIdata/Cardiac/',scanner,'/',Patient,'/Processing'));
        for subset=1:3
...
for seriesnum=seriesnumStart:seriesnumEnd
  slice=seriesnum-seriesnumStart+1
load(strcat('/v/raid1/', MRIdataOwner, '/MRIdata/Cardiac/',scanner,'/',Patient,'/Processing/Output/MultiSRT_Vals_M0_fA_T1_chi2err.series',int2str(seriesnum*1000),'.slice',int2str(slice),'.mat'));
...
seriesnumEnd=29
% must also change filenames
scan_name='CV_Radial7Off_triple_4.1ml('
MID=55

if doReconFlag==1
    load(['/v/raid1/bmatthew/MRIdata/Cardiac/' scanner '/' Patient '/RawData/meas_MID55_CV_Radial7Off_triple_4.1ml_FID11093_Kspace'],'-mat');
end
...
seriesnumEnd=32
% must also change filenames
scan_name='CV_Radial7Off_triple_6.2ml_adeno('
MID=57
if doReconFlag==1
    load(['/v/raid1/bmatthew/MRIdata/Cardiac/' scanner '/' Patient '/RawData/meas_MID57_CV_Radial7Off_triple_6.2ml_adeno_FID11095_Kspace'],'-mat');
end
...
seriesnumEnd=65
% must also change filenames
scan_name='CV_Radial7Off_triple_6.2ml_lexi('
MID=76
if doReconFlag==1
    load(['/v/raid1/bmatthew/MRIdata/Cardiac/' scanner '/' Patient '/RawData/meas_MID76_CV_Radial7Off_triple_6.2ml_lexi_FID11114_Kspace'],'-mat');
end
